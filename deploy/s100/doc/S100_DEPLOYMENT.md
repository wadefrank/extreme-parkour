...
全 INT16+EC 后，depth encoder 的三个输出均可控制在约 `0.003` 以内。Actor
可能在极少数大幅值动作上仍出现约 `0.14` 的原始输出误差，但控制运行时会先
将动作裁剪到 `[-4.8, 4.8]`，再乘以 `action_scale=0.25`。因此 HBM 验收默认
同时报告原始动作误差，并以裁剪后的动作为通过条件；原始值位于同一饱和侧时，
